...
I first bring in my dataset and set all empty data to NA. I then format my date column to be of type date and derive a Year column for potential use in visualizations. Additionally, I created a new column of data for the number of drugs within the person's system at the time of death, currently set to 0. By setting the number of drugs to 0, this will ease the process of adding 1 for each column variable that was present. I include the head of my dataframe as a reference to changes made afterwards.
```{r}
...
I now include visualizations to the drug overdose deaths, each explained within its graph title. I use filtering to promote cleaner data and reduce outliers throughout (mostly in terms of eliminated NA's). I left these values in currently to refrain from eliminating data I may want to use in the future.
```{r}
filter(df, !is.na(Age)) %>%
  ggplot(aes(x=Age)) + 
...
Within the dataset, 2016 and some of 2014 do not have location data for the death county, so I will be working towards deriving this information from those that do include death city with another dataset that links cities to counties for Connecticut in the next deliverable.
```{r}
...
<h1>Conclusion</h1>
The current data shows trends in the increased involvement in Fentanyl in overdoses, as well as an increased number of drugs found in the system of those who overdosed. As there is a lot of missing data within the current dataset, in deliverable two I will incorporate more datasets to fill in much of the missing data to create models and visualizations that accurately depict the overdose problem particular to cities and counties of Connecticut.